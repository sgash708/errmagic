...
![errmagic アーキテクチャ全体図](docs/architecture-diagram.ja.png)
...
[`examples/`](examples/README.md) に最小のエンドツーエンド構成があります: errmagic を組み込んだ Vite フロントエンド、リプレイをローカルディスクに保存する Hono 受信API、S3 バケットの Terraform です。front + api はクラウド不要でローカルだけで動き、エラーを発生させて同梱ビューアでリプレイ再生するまで一周できます。
